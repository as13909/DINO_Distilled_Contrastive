...
class StudentFeatureExtractor:
    ...
    Only uses backbone embeddings, ignoring the projection head.
    """

    def __init__(self, ckpt_path, use_timm, student_model, proj_dim=1024,
                 img_size=96, device='cuda'):
        self.device = device
        ...
        state_dict = ck.get("student_state", ck.get("state_dict", ck))

        # Remove head keys to avoid mismatch
        ...
        print("  Using backbone embeddings only (ignoring head).")
    ...
    def extract_features(self, pil_image):
        ...
        with torch.no_grad():
            # Only use the backbone's pooled feature
